      ...
      buttons = (
        <>
          <Button
            ...
            onClick={this.clickMorning}
            active={this.state.show_morning}
            variant=""
            size="lg"
          >
            Morning
          ...
          <Button
            href="#start"
            className={styles.button}
            onClick={this.clickEvening}
            active={this.state.show_evening}
            variant="light-green"
            size="lg"
          >
            Evening
    ...
    return (
      <div>
        <div className={styles.landing_wrapper}>
          ...
          <Row>
            <Col className={styles.landing_col}>{buttons}</Col>
          </Row>
        </div>
        ...
        {thelist.map((list) => (
          <>
            <Row className={styles.post_wrapper}>
              <Col
                className={styles.post_inner_wrapper}
                xs={{ span: 10, offset: 1 }}
                sm={{ span: 8, offset: 2 }}
              >
                <Row className={styles.post_id}>
                  <h2>{list.id}</h2>
                </Row>
                <Row className={styles.post_text}>
                  <p key={list.id} align="right" dir="rtl">
                    {list.text}
                  </p>
                </Row>
                <Row className={styles.greentext}>
                  <Col className={styles.post_footer}>
                    <p>
                      Read <span className={styles.bold}>{list.times_int}</span>{" "}
                      {list.times_int > 1 ? "times" : "time"}
                    </p>
                    <p>{list.source}</p>
                  </Col>
                </Row>
